...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    def _rate_limit_check(self, request: Request) -> Optional[Response]:
        ...
        self._rate_limit_storage = {
            ip: hits
            for ip, hits in self._rate_limit_storage.items()
            if hits["timestamp"] > window_start
        }
        ...
        if self._rate_limit_storage[client_ip]["count"] > 60:  # 60 requests per minute
            return Response(
                status_code=429,
                description=b"Too Many Requests",
    ...
    def _verify_jwt_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise Exception(details={"message": "Token has expired"})
        except jwt.InvalidTokenError:
            raise Exception(details={"message": "Invalid token"})
    ...
    def _generate_csrf_token(self, session_id: str) -> str:
        ...
        signature = hmac.new(
            self._secret_key, token_data.encode(), hashlib.sha256
        ).digest()
        return b64encode(
    ...
    def _validate_csrf_token(self, token: str) -> bool:
        ...
        try:
            ...
            expected_signature = hmac.new(
                self._secret_key, expected_data.encode(), hashlib.sha256
            ).digest()
    ...
    async def before_request(self, request: Request) -> Request | Response:
        ...
        if self.secur_config.csrf_protection and request.method in [
            ...
            if not csrf_token or not self._validate_csrf_token(csrf_token):
                return Response(
                    content={"message": "Invalid CSRF token"}, status_code=401
                )
        return request
    ...
    async def dispatch(self, request, call_next):
        result = await self.before_request(request=request)
        if isinstance(result, Response):
            return result
        response = await super().dispatch(request, call_next)
        return await self.after_request(response)
