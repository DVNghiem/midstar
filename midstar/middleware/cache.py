...
class EdgeCacheMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, cache_config: CacheConfig | None = None, *args, **kwargs):
    ...
    def before_request(self, request: Request) -> Request | Response:
        ...
        if etag:
            ...
            if if_none_match and if_none_match == etag:
                return Response(status_code=304, content=b"", headers={"ETag": etag})
        return request

    def after_request(self, request: Request, response: Response) -> Response:
        """Add caching headers to response"""
        if not self._should_cache(request, request.url.path):
        ...
        response.headers.update(
            {
                ...
                "Last-Modified": datetime.now(tz=timezone.utc).strftime(
                    "%a, %d %b %Y %H:%M:%S GMT"
                ),
            }
        )
        ...
        response.headers.set(
            "Surrogate-Control",
            f"max-age={self.cache_config.s_maxage or self.cache_config.max_age}",
        )
        return response
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        return self.after_request(request, response)
